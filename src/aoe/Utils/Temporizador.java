...
public class Temporizador {
    ...
    public Temporizador( MinaOro minaEspanol, Carpinteria carpinteriaEspanol) {
            TimerTask tarea = new TimerTask() {
                ...
                public void run() {
                    ...
                    if (tiempo % 1 == 0) {
                        ConseguirRecursos recursos = new ConseguirRecursos(minaEspanol,carpinteriaEspanol);
                        recursos.start();
                    }
                    //Si han pasado 5 segundos se hace esto
                    if (tiempo % 5 == 0) {
                       Batallas batalla = new Batallas(minaEspanol,carpinteriaEspanol);
                       batalla.start();
                    }
                    
                    //Si han pasado 20 segundos se hace esto
                    if (tiempo % 20 == 0) {
                        Reponedor reponedor = new Reponedor(carpinteriaEspanol,minaEspanol);
                        reponedor.start();
                    }
